...
const mockMovieDetails = {
    imageSrc: chinatsu,
    title: "青いの箱",
    description: "本作は、スポーツ名門校である栄明学園を舞台に、男子バドミントン部員の猪股大喜と、女子バスケットボール部のスター選手である鹿野千夏、そして新体操部の蝶野雛との青春ラブストーリーを描いています。大喜は毎朝体育館で自主練習を行い、同じく朝練をしている千夏に密かな想いを寄せています。ある日、千夏の両親が海外赴任することになり、彼女は大喜の家に居候することに。同じ屋根の下で生活する中で、大喜は千夏にふさわしい存在になるため、全国大会出場を目指して努力を重ねます。一方、幼馴染の雛もまた、大喜に特別な感情を抱いており、三人の関係性が物語の中で繊細に描かれています。",
    genre: "romcom",
    releaseDate: "12/12/2024",
    duration: "120p"
} as IMovieDetails;

export const MovieDetailsUI: React.FC = () => {
    const { id } = useParams();
    console.log(id)
    ...
    return (
        <>
            <Layout>
                <Content style={{ paddingTop: "-5rem" }}>  {/* Adjust padding to offset the fixed header */}
                    <Row
                        gutter={[16, 0]}
                        justify="center"
                        align="middle"
                        style={{ marginLeft: "20rem", marginRight: "20rem", minHeight: "100vh", display: "flex" }}
                    >
                        {/* Image Column */}
                        ...
                        <Col xs={24} md={14} style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
                            <div style={{ marginTop: "-1rem" }}>  {/* Reduce default spacing */}
                                ...
                                <Title level={5} style={{ marginBottom: "0.25rem" }}>
                                    {t("labels.titles.release_date")} : {mockMovieDetails.releaseDate}
                                </Title>
